...
INPUT_PATH = "/tmp/discussion-segment-tightened-llm-selected-20260626.json"
OUTPUT_PATH = "/tmp/discussion-segment-tightened-llm-results-20260626.json"
PROVIDER_ID = 10
MODEL_ID = 27
MODEL_NAME = "gpt-5.5"
...
def prompt(segment):
    payload = {
        ...
        "chat_metadata": {
            "chat": segment.get("chat"),
            "topic": segment.get("topic"),
            "telegram_chat_id": segment.get("telegram_chat_id"),
            "forum_topic_id": segment.get("forum_topic_id"),
            "message_thread_id": segment.get("message_thread_id"),
        },
        "combined_score": segment["combined_score"],
        "signal_families": segment.get("signals", []),
        "proposed_material_type": segment["proposed_type"],
    ...
    return (
        ...
        "For health, employment, security, legal, finance, or risk topics, accept only with careful source-aware wording and no unsupported claims. "
        "Reject low-value adjacent windows, duplicate fragments, context without solution, and bare entity repetition.\n"
...
def insert_provider_call(segment, prompt_text, response_json, parsed, status, http_status, latency_ms, error):
    ...
    response_payload = {
        "segment_id": segment["segment_id"],
        "fixture_or_new": segment["fixture_or_new"],
        "raw_ids": segment["raw_ids"],
        "proposed_type": segment["proposed_type"],
        "combined_score": segment["combined_score"],
        "parsed": parsed,
    }
    sql = f"""
...
def main():
    ...
    if len(selected) > 10:
        raise SystemExit("selected segment count exceeds 10")
    ...
    for segment in selected:
        ...
        call_id = insert_provider_call(segment, prompt_text, response_json, parsed, status, http_status, latency_ms, error)
        results.append({
            **segment,
    ...
    print(json.dumps({"selected": len(selected), "provider_calls": len(results), "segment_ids": [item["segment_id"] for item in selected]}, ensure_ascii=False))
